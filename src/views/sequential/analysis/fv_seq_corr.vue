      ...
      <el-col class="mb20">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            <el-select v-model="select_type" placeholder="请选择" @change="changeCorrData">
              <el-option

                v-for="key in Object.keys(type_list)"
                :key="key"
                :label="key"
                :value="key">
              </el-option>
...
import sequential from "../../../api/factor/sequential";
import Table from "@/views/components/factor/table";

var myChart
export default {
  name: 'fv_seq_analy_corr',
      components: {
    Table
  },
  data() {
  ...
  methods: {
    changeCorrData(val){
      console.log(val)
      this.factor_item = this.type_list[val]
      this.getEcharts()
    },
    ...
    if (that.tableName === 'fac_table' || that.tableName === 'sga_table') {
      ...
      this.$api.sequential.calFvSeqCorr(param).then(res => {
        if ((Date.now() - cur_time) / 1000 > 60) {
          that.$notice('相关性分析', "计算完成,请返回浏览器查看详情！")
        }

        that.type_list = res
        that.select_type = Object.keys(res)[0]
        that.factor_item = res[that.select_type]
        that.getEcharts()
        loading.close()
