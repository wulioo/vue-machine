        ...
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            ...
            <div style="float:right;display: flex;align-items: center;">
              <el-button size="small" type="primary" icon="el-icon-download" @click="exportExcel()">收益曲线</el-button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    backtesting() {
      ...
      this.$api.layered.getBackTesting(parma).then(res => {
    ...
    exportExcel() {
      ...
      //json转sheet
      let table_arr = []
      for (let i in this.export_table['x_data']) {
        let temp_dict = {'trading_date': this.export_table['y_data'][i],'label':this.export_table['x_data'][i]}
        table_arr.push(temp_dict)
      }
      var ws = XLSX.utils.json_to_sheet(table_arr, {});
      ...
      //输出
      XLSX.writeFile(wb, this.t_name + `_${this.factor}_${this.mapLabel[this.factor_label]}.csv`);
    },
    getEchartsData() {
      let that = this
